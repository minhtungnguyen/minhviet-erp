...
function TourGhepProductForm({ initial, onSave, onCancel, suppliers }) {
  const empty = {
  ...
  return (
    <div style={{ background:"#fff", borderRadius:14, padding:24, border:"0.5px solid #e2e8f0", marginBottom:20 }}>
      <h3 style={{ margin:"0 0 20px", fontSize:15, fontWeight:600 }}>
        {initial ? "Sửa sản phẩm" : "Thêm sản phẩm tour ghép"}
      </h3>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        ...
        <div style={{ gridColumn:"1/-1" }}>
          <div style={{ display:"flex", gap:8, marginBottom:10 }}>
            {[
              { val:"fixed",      label:"Giá cố định",  desc:"Đối tác có bảng giá ổn định" },
              { val:"negotiable", label:"Thương lượng",  desc:"Giá thay đổi theo số lượng/thời điểm" },
            ].map(o=>(
              <button key={o.val} type="button" onClick={()=>set("priceType",o.val)}
                style={{ flex:1, padding:"10px 14px", borderRadius:8, cursor:"pointer",
                  border:`1.5px solid ${form.priceType===o.val?"#185FA5":"#e2e8f0"}`,
                  background:form.priceType===o.val?"#E6F1FB":"#fff", textAlign:"left" }}>
                <div style={{ fontSize:13, fontWeight:600, color:form.priceType===o.val?"#185FA5":"#374151" }}>{o.label}</div>
          ...
          <table style={{ width:"100%", borderCollapse:"separate", borderSpacing:0, border:"0.5px solid #e2e8f0", borderRadius:10, overflow:"hidden", fontSize:13 }}>
            ...
            <tbody>
              {[
                ...
                { key:"child",  label:"Trẻ em (5–11t)" },
                { key:"infant", label:"Em bé (<5t)" },
              ].map(row=>{
...
function TourGhepProductCard({ product:p, onEdit, onSelect, orderCount, canSeeSecret, canEdit }) {
  const profit = (p.sellPrices?.adult||0)-(p.buyPrices?.adult||0);
  const margin = p.sellPrices?.adult>0 ? (profit/p.sellPrices.adult*100).toFixed(1) : 0;
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
          {canSeeSecret ? (
            <div style={{ padding:"8px 10px", background:"#fff5f5", borderRadius:8, fontSize:11 }}>
              <div style={{ color:"#94a3b8", marginBottom:2 }}>Giá mua NL</div>
              <div style={{ fontWeight:600, color:"#A32D2D" }}>
                {(p.buyPrices?.adult||0).toLocaleString("vi-VN")}đ
              </div>
            </div>
          ) : (
          ...
          <div style={{ padding:"8px 10px", background:"#f0fdf4", borderRadius:8, fontSize:11 }}>
            <div style={{ color:"#94a3b8", marginBottom:2 }}>Giá bán NL</div>
            <div style={{ fontWeight:600, color:"#0F6E56" }}>
              {(p.sellPrices?.adult||0).toLocaleString("vi-VN")}đ
            </div>
          </div>
        </div>
