...
const newMod = fs.readFileSync('C:/minhviet-erp/new_hdv.jsx', 'utf8');
let start=-1;
for(let i=0;i<appLines.length;i++){
  if(/^function HDVModule\(/.test(appLines[i])){ start=i; break; }
}
if(start<0){ console.error('HDVModule not found'); process.exit(1); }
