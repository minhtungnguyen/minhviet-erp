...
const newMod = fs.readFileSync('C:/minhviet-erp/new_orderform.jsx', 'utf8');
let start=-1;
for(let i=0;i<appLines.length;i++){
  if(/^function OrderForm\(/.test(appLines[i])){ start=i; break; }
}
if(start<0){ console.error('OrderForm not found'); process.exit(1); }
