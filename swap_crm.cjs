...
const newMod = fs.readFileSync('C:/minhviet-erp/new_crm.jsx', 'utf8');
let start=-1;
for(let i=0;i<appLines.length;i++){
  if(/^function CrmModule\(/.test(appLines[i])){ start=i; break; }
}
if(start<0){ console.error('CrmModule not found'); process.exit(1); }
