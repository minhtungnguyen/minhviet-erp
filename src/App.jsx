...
function buildPhieuThu(v, order) {
...
function buildPhieuChi(v, order) {
...
function buildConfirmation(order, vouchers, tourOp) {
  const paid = vouchers
...
function PrintBtn({ onClick, label="🖨 In phiếu", small=false }){

  return(

    <button onClick={onClick}
      style={{padding:small?"4px 10px":"7px 14px",fontSize:small?11:12,fontWeight:600,cursor:"pointer",
        borderRadius:8,border:"1px solid #bfdbfe",background:"#eff6ff",color:"#1e3a8a",
...
const PERM_LABEL = Object.fromEntries(PERMISSION_GROUPS.flatMap(g=>g.items.map(i=>[i.key,i.label])));
...
function SaleDashboard({ currentUser, orders=[], vouchers=[], quotes=[], personalTargets=[], careTasks=[], bookings=[], setView, setSelected }){
  // ── Helpers ──────────────────────────────────────────────
...
function DirectorDashboard({ orders=[], vouchers=[], expenses=[], personalTargets=[], userAccounts=[], customers=[], setView, bankAccounts=[], bookings=[], quotes=[] }){
  // ── Helpers ──────────────────────────────────────────────
  const fmt  = (n) => (n || 0).toLocaleString("vi-VN") + "đ";
  const fmtM = (n) => {
    const a=Math.abs(n||0),s=(n||0)<0?"-":"";
...
function AccountantDashboard({ orders=[], vouchers=[], expenses=[], refunds=[], credits=[], bankAccounts=[], setView, onViewOrder }){
  // ── Helpers ──────────────────────────────────────────────
  const fmt  = (n) => (n || 0).toLocaleString("vi-VN") + "đ";
  const fmtM = (n) => {
    const a=Math.abs(n||0),s=(n||0)<0?"-":"";
...
function CrmModule({orders,pushNotif,customers:customersProp=SEED_CUSTOMERS,onUpdateCustomers,currentUser,msgHistory,onLogMessage,onCreateOrderFromLead,onViewOrder}){
...
function ApprovalsModule({ orders, expenses, vouchers=[], onExpenseUpdate, onVoucherUpdate, pushNotif, currentRole, currentUser }){
...
function ReportModule({ orders, vouchers, expenses, personalTargets=[], currentRole, hdvList=[], customers=[], userAccounts=[], bookings=[] }){
  const [tab,setTab]=React.useState("overview");
  const [period,setPeriod]=React.useState("month");
...
export default function App(){
  ...
  return (
    ...
    <div key={view} className="page-enter">
      {view==="dashboard"&&isBanGiamDoc(currentRole)&&<DirectorDashboard orders={orders} vouchers={vouchers} expenses={expenses} personalTargets={personalTargets} onUpdateTargets={setPersonalTargetsP} userAccounts={userAccounts} customers={customers} setView={setView} bankAccounts={bankAccounts} bookings={bookings} quotes={quotes||[]}/>}
