...
const newMod = fs.readFileSync('C:/minhviet-erp/new_aftersale.jsx', 'utf8');
let start=-1;
for(let i=0;i<appLines.length;i++){
  if(/^function AfterSaleModule\(/.test(appLines[i])){ start=i; break; }
}
if(start<0){ console.error('AfterSaleModule not found'); process.exit(1); }
